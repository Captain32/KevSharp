...
<%
    ...
    String sqlpath = "webvideo/";
    //data type
    String contentType = request.getContentType();
    //String title = (String)(request.getParameter("title"));
	//String block_id = (String)(request.getParameter("block_id"));
	String video_user_id = (String)session.getAttribute("user_id");
    ...
    try {
        ...
        if (contentType.indexOf("multipart/form-data") >= 0) {
          ...
          //检查上传文件的目录是否存在
			...
			String sql = "select user_name from users where user_id=" + video_user_id;
			rst = stmt.executeQuery(sql);
			rst.next();
			String video_user_name = rst.getString("user_name");	             
			sql = "select video_id from videos where video_user_id ="+video_user_id+" order by video_id desc limit 1";
    		rst = stmt.executeQuery(sql);
    		int max_id = 0;
    		if(rst.next())
    			max_id = Integer.valueOf(rst.getString("video_id")).intValue()+1;	
    		sql = "insert into videos(block_id,video_user_id,video_user_name,title,path,visit_num,time) values("
    				+ block_id + "," + video_user_id + ",'" + video_user_name + "','" + title + "','" + sqlpath +filename + "'," + 0 + ",current_timestamp())";
